...
async (malvin, mek, m, { from, quoted, isGroup, isAdmins, isCreator, fromMe, reply }) => {
    try {
        ...
        const groupData = await malvin.groupMetadata(from);
        const presencePromises = [];

        // Request presence updates for all participants
        for (const participant of groupData.participants) {
            presencePromises.push(
                malvin.presenceSubscribe(participant.id)
                    .then(() => {
                        // Additional check for better detection
                        return malvin.sendPresenceUpdate('composing', participant.id);
                    })
            );
        }
